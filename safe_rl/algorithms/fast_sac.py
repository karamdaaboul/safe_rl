...
Brings the key optimizations from fzi_rl's FastSAC into safe_rl's ecosystem:
...
def _critic_loss_standard(
    ...
    with torch.no_grad():
        next_actions, next_log_prob = actor.sample(actor_obs_normalizer(next_obs))

        next_obs_norm = critic_obs_normalizer(next_obs)
        next_input = torch.cat([next_obs_norm, next_actions], dim=-1)
        q1_target = target_1(next_input)
        q2_target = target_2(next_input)
    ...
    q_input = torch.cat([obs_norm, actions], dim=-1)
    q1 = critic_1(q_input)
    q2 = critic_2(q_input)
...
def _actor_loss_fn(
    ...
    actions, log_prob = actor.sample(actor_obs_normalizer(obs))
    ...
    def update(
        ...
        for update_idx in range(self.num_updates_per_step):
            ...
            if update_idx % self.policy_frequency == 0:
                actor_loss, log_pi_mean = self._update_actor(obs, alpha_val)
        ...
        with autocast(device_type=self.amp_device, dtype=self.amp_dtype, enabled=self.amp):
            if self.policy.is_distributional_critic:
                loss = _critic_loss_distributional(
                    ...
                    alpha, self.gamma, self.clipped_double_q,
    ...
    def _update_actor(
        ...
        with autocast(device_type=self.amp_device, dtype=self.amp_dtype, enabled=self.amp):
            actor_loss, log_pi_mean = _actor_loss_fn(
                obs, alpha, self.clipped_double_q,
