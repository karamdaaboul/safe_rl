...
class REPPO:
    ...
    def _update_actor(
        ...
        # REPPO actor loss (reference, arXiv:2507.11019, Eq. for the policy
        # objective): smooth Lagrangian penalty rather than a hard KL gate.
        #   L = α_temp·logπ − q_pi + α_kl·KL
        # The pathwise reward term (−q_pi) is always active, so the policy keeps
        # improving; α_kl (learned by the dual below) softly regulates step size
        # to hold KL near desired_kl. The previous torch.where gate dropped the
        # −q_pi term entirely whenever KL exceeded the bound, stalling progress.
