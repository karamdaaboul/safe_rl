...
class P3O(PPO):
    ...
    def __init__(
        ...
        if getattr(self.policy, "is_distributional_cost_critic", False) and self.use_clipped_cost_loss:
            print(
                "[P3O] use_clipped_cost_loss=True is incompatible with a distributional "
                f"({self.policy.cost_critic_loss_type}) cost critic; forcing use_clipped_cost_loss=False."
            )
    ...
    def _update_policy(
        ...
        if getattr(self.policy, "is_distributional_cost_critic", False):
            ...
            # Saturation diagnostic: fraction of cost-return targets pinned at v_max. ~0 means
            # the support is sized fine; sustained > 0 means raise v_max or use symlog support.
            with torch.no_grad():
                batch_metrics["cost_return_clip_frac"] = float(
                    self.policy.cost_critic.clip_fraction(returns_cost_batch).item()
                )
